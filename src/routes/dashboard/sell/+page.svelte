...
<main class="min-h-screen bg-[#f7f4ee] py-8">
  <div class="mx-auto max-w-6xl space-y-6 px-4 sm:px-6">
    <header class="flex flex-wrap items-end justify-between gap-4">
      <div><a href="/dashboard" class="text-sm font-semibold text-[#18372f]">← Dashboard</a><p class="mt-4 text-xs font-bold uppercase tracking-[0.18em] text-[#a95739]">Seller workspace</p><h1 class="mt-1 text-3xl font-black text-slate-950">Create a lot</h1><p class="mt-1 text-sm text-slate-500">Start with a photo or enter the catalog details yourself.</p></div>
      <div class="flex flex-wrap gap-2"><a href="/dashboard/sell/bulk" class="inline-flex items-center gap-2 rounded-sm border border-[#ddd6ca] bg-white px-5 py-3 font-bold text-[#18372f] shadow-sm transition hover:bg-[#efe8dc]">Bulk CSV / Grid</a><button type="button" onclick={() => document.getElementById('ai-image')?.click()} disabled={imageBusy || !profileReady} class="inline-flex items-center gap-2 rounded-sm bg-[#18372f] px-5 py-3 font-bold text-white shadow-sm transition hover:bg-[#152c26] disabled:cursor-not-allowed disabled:opacity-50"><span aria-hidden="true">✦</span>{imageBusy ? 'Reading image…' : 'Create from image'}</button></div>
    ...
    {#if problem}<div class="rounded-sm border border-red-200 bg-red-50 p-4 text-sm font-medium text-red-800">{problem}</div>{/if}
    {#if message}<div class="rounded-sm border border-[#c5d0c8] bg-[#e8eee9] p-4 text-sm font-medium text-[#18372f]">{message}</div>{/if}

    {#if loading}
      <div class="rounded-sm border bg-white py-20 text-center text-slate-500">Loading seller workspace…</div>
    {:else}
      <section class="rounded-sm border bg-white shadow-sm">
        <div class="flex flex-wrap items-center justify-between gap-3 p-5">
          <div class="flex items-center gap-3"><span class="grid h-9 w-9 place-items-center rounded-sm {profileReady ? 'bg-emerald-100 text-[#18372f]' : 'bg-amber-100 text-amber-700'}">{profileReady ? '✓' : '!'}</span><div><h2 class="font-bold text-slate-950">Seller details</h2><p class="text-sm text-slate-500">{profileReady ? `${profile.displayName} · ready to submit` : 'Complete these details before saving a lot'}</p></div></div>
        ...
        {#if profileOpen}
          <form class="grid gap-4 border-t bg-[#f7f4ee] p-5 sm:grid-cols-2" onsubmit={(event) => { event.preventDefault(); saveProfile(); }}>
        ...
        <form class="grid gap-6 p-5 lg:grid-cols-[280px_minmax(0,1fr)]" onsubmit={(event) => { event.preventDefault(); saveLot(false); }}>
          <div>
            {#if lot.images.length}
              <div class="space-y-3">{#each lot.images as image, index}<div class="overflow-hidden rounded-sm border bg-slate-100"><img src={image.previewUrl || image.url} alt={`Listing reference ${index + 1}`} class="aspect-square w-full object-cover" /></div>{/each}<button type="button" onclick={() => document.getElementById('ai-image')?.click()} disabled={imageBusy || lot.images.length >= 8} class="w-full rounded-lg border border-[#ddd6ca] px-3 py-2 text-sm font-bold text-[#18372f] disabled:opacity-50">+ Analyze another photo</button></div>
            {:else}
              <button type="button" onclick={() => document.getElementById('ai-image')?.click()} disabled={imageBusy || !profileReady} class="grid aspect-square w-full place-items-center rounded-sm border-2 border-dashed border-[#ddd6ca] bg-[#efe8dc] p-6 text-center text-[#18372f] disabled:opacity-50"><span><span class="block text-4xl">✦</span><strong class="mt-3 block">Create from image</strong><span class="mt-1 block text-sm text-[#a95739]">Take a photo or choose one. AI will draft the listing.</span></span></button>
            {/if}
          ...
          <div class="space-y-4">
            ...
            {#if policy}
              <details class="rounded-sm border border-[#ddd6ca] bg-[#efe8dc] p-4"><summary class="cursor-pointer font-bold text-blue-950">Pumbi seller terms · {ratePercent(policy.sellerCommissionRate)} commission</summary><div class="mt-3 max-h-48 overflow-y-auto whitespace-pre-wrap rounded-lg bg-white p-3 text-sm text-slate-700">{policy.sellerTerms}</div><label class="mt-3 flex items-start gap-2 text-sm font-semibold text-blue-950"><input type="checkbox" class="mt-1 rounded" bind:checked={acceptedTerms} /><span>I accept version {policy.version} of the Pumbi seller terms and rates.</span></label></details>
            {:else}<div class="rounded-sm border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">No active seller policy is published. You can save drafts, but submitting is temporarily unavailable.</div>{/if}

            <div class="flex flex-wrap gap-3"><button type="submit" disabled={busy || imageBusy} class="rounded-lg border px-4 py-2 font-bold disabled:opacity-50">Save draft</button><button type="button" disabled={busy || imageBusy || !policy || !acceptedTerms} onclick={() => saveLot(true)} class="rounded-lg bg-[#18372f] px-4 py-2 font-bold text-white disabled:cursor-not-allowed disabled:opacity-50">Submit for review</button></div>
          </div>
        </form>
      </section>

      <section class="rounded-sm border bg-white p-5 shadow-sm"><div class="flex items-center justify-between"><div><h2 class="text-xl font-black">Your submissions</h2><p class="text-sm text-slate-500">Drafts and review progress</p></div><span class="rounded-sm bg-slate-100 px-3 py-1 text-xs font-bold">{submissions.length}</span></div>{#if submissions.length === 0}<div class="py-10 text-center text-sm text-slate-500">You have not created any lot submissions yet.</div>{:else}<div class="mt-5 grid gap-3 md:grid-cols-2">{#each submissions as item}<article class="flex gap-4 rounded-sm border p-4">{#if item.images?.[0]}<img src={item.images[0].previewUrl} alt={item.title || 'Lot submission'} class="h-20 w-20 shrink-0 rounded-lg object-cover" />{:else}<div class="grid h-20 w-20 shrink-0 place-items-center rounded-lg bg-slate-100 text-2xl text-slate-300">◇</div>{/if}<div class="min-w-0 flex-1"><div class="flex items-start justify-between gap-2"><h3 class="truncate font-bold">{item.title || 'Untitled draft'}</h3><span class={`rounded-sm px-2 py-0.5 text-[10px] font-bold ${item.status === 'APPROVED' ? 'bg-emerald-100 text-[#18372f]' : item.status === 'REJECTED' ? 'bg-red-100 text-red-800' : item.status === 'SUBMITTED' ? 'bg-blue-100 text-[#18372f]' : 'bg-slate-100 text-slate-700'}`}>{item.status}</span></div><p class="mt-1 truncate text-xs text-slate-500">{item.auctionSeries?.name || item.auction?.title || 'Opportunity unavailable'}</p>{#if item.rejectionReason}<p class="mt-2 text-xs text-red-700">{item.rejectionReason}</p>{/if}<div class="mt-3 flex gap-3 text-xs"><span class="text-slate-400">Updated {formatDate(item.updatedAt)}</span>{#if item.status === 'DRAFT' || item.status === 'REJECTED'}<button type="button" class="font-bold text-[#18372f]" onclick={() => editSubmission(item)}>Edit</button>{/if}</div></div></article>{/each}</div>{/if}</section>
    {/if}
  </div>
</main>
